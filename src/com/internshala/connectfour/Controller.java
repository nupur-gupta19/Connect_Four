...
public class Controller implements Initializable {
	...
	public void createPlayground(){
        Shape playground=createGameStructuralGrid();
     ...
     private Shape createGameStructuralGrid(){
	     ...
	     for (int row=0;row<ROWS;row++){
		     for (int col=0;col<COLUMNS;col++){
			     ...
			     circle.setRadius(CIRCLE_DIAMETER/2f);
			     circle.setCenterX(CIRCLE_DIAMETER/2f);
			     circle.setCenterY(CIRCLE_DIAMETER/2f);
				 circle.setSmooth(true);
			     circle.setTranslateX(col*(CIRCLE_DIAMETER+5)+CIRCLE_DIAMETER/4f);
			     circle.setTranslateY(row*(CIRCLE_DIAMETER+5)+CIRCLE_DIAMETER/4f);
     ...
     private List<Rectangle> createClickableColumns(){
		...
		for (int col=0;col<COLUMNS;col++) {
			 ...
			 rectangle.setTranslateX(col*(CIRCLE_DIAMETER+5)+CIRCLE_DIAMETER/4f);
     ...
     private void  insertDisc(Disc disc,int column){
		...
		disc.setTranslateX(column*(CIRCLE_DIAMETER+5)+CIRCLE_DIAMETER/4f);
		int currentRow=row;
	     TranslateTransition translateTransition=new TranslateTransition(Duration.seconds(0.5),disc);
		translateTransition.setToY(row*(CIRCLE_DIAMETER+5)+CIRCLE_DIAMETER/4f);
    ...
    public void resetGame(){
     ...
     private static class Disc extends Circle{
		...
		public Disc(boolean isPlayerOneMove){
			...
			setRadius(CIRCLE_DIAMETER/2f);
			setFill(isPlayerOneMove?Color.valueOf(discColour1):Color.valueOf(discColour2));
			setCenterX(CIRCLE_DIAMETER/2f);
			setCenterY(CIRCLE_DIAMETER/2f);
